...
3. In §3 set the two image paths. `SEED = None` draws a random seed and prints it.
   ...
   on a flat-lay the bald pass invents one (`prd/v3/v3.8/BUILD.md` §6.1d). The test is a
   **head** check — Selfie Multiclass FACE + HAIR, at least 500 px, one model the crop
   already loads — and it made no error either way over 56 worn and 17 person-free
   photographs.
