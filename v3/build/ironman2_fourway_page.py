...
const F={'f-fail':'f-fail','f-bad':'f-bad','f-all':''};
Object.keys(F).forEach(id=>{document.getElementById(id).onclick=()=>{
  document.body.classList.remove('f-fail','f-bad');
...
document.getElementById('export').onclick=()=>{let csv='set_id,seed,vei_verdict,best_arm\\n';
 blocks.forEach(b=>{csv+=b.dataset.sid+','+b.dataset.seed+','+(b.dataset.vd||'')+','+(votes[b.dataset.sid+'|'+b.dataset.seed]||'VEi')+'\\n';});
