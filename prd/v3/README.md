...
| **Failure records — where they live** | Three different things get called "the failures" and they are not the same set. **v3.3's** failures: `v3/testsets/v34_failures.csv`, cut from `v33_ironman_votes_bca4.csv`. **The v3.4 lock's own** (`VEi`): `v34_im2_truth.json` — 600 cells, 100% human-judged, 55 FAIL / 70 MID / 475 CLEAN, indexed in [v3.4 RESULTS](v3.4/RESULTS.md#where-the-failure-records-live). **`BC_klein`'s own**: `v3/testsets/bc_count.csv` — the same 600 cells marked in one pass on `v3/report/bc_count.html` (2026-09-10), **29 FAIL (4.8%)**, 19 pairs, 2 seed-stable; **re-marked** by the same reviewer on the same page as `bc2_count.csv`, **50 FAIL (8.33%)** — the two passes agree on 94.8% of cells but only **Jaccard 0.44** on failures, so a rate marked in one sitting cannot be compared with a rate marked in another ([v3.8 RESULTS §3](v3.8/RESULTS.md)). **`ER`'s own**: `v3/testsets/er_count.csv`, 18 FAIL on the lenient footing. The union of the first two is `v3/testsets/v35_linkC.csv` (51 pairs), which v3.5 link C runs on. Counting pages are rebuilt with `python3 v3/build/bc_count_page.py [BC\|VEi]`. |
...
| **v3.5 — open** | what does call 1 actually have to do to the reference — and can the head come off instead of being replaced? Separates the lock's mannequin sentence from its re-pose sentence; the target shape is **VEi's re-pose with BC's head removal**. Link A run 2026-09-08 (56 garments × 4 call-1 arms, fal, seed 46). [experiment](v3.5/EXPERIMENT.md) · [matrix](v3.5/TEST.md) · [results](v3.5/RESULTS.md) · page `v3/report/v35_linkA.html` |
| **v3.7 — concluded, negative** | can klein re-pose the garment's wearer to match a photograph of the target person, handed to call 1 as a second image? **No** — the wearer comes back in the source pose on most of the fold, and on about a third klein returns the *target's* photograph with the garment swapped in. The second image is read as a source of clothing and scene, not of pose; both wordings behave the same. Strictly worse than the `BC` it modifies, and 21% more calls per pair. [experiment](v3.7/EXPERIMENT.md) · [matrix](v3.7/TEST.md) · [results](v3.7/RESULTS.md) · page `v3/report/v37.html` |
| **v3.8 — LOCKED 2026-09-10** | how much is left in call 2's prompt? **One verb.** `ER` — *replace the clothing in image 1 with* in place of *dress the person in* — repairs **32%** of `BC_klein`'s failures (16/50, CI 21–46%), implying **8.33% → 6.17%**, at no extra call, model or measurable time. Every longer prompt tested was neutral or harmful, and V2's *never name a body part the crop excludes* is confirmed on call 2 with a number (feet invented on 21 of 59 vs 2). The other **68%** of failures are reference-side and unreachable by wording, which closes call-2 prompting the way v3.2 closed iterating the edit. [experiment](v3.8/EXPERIMENT.md) · [matrix](v3.8/TEST.md) · [results](v3.8/RESULTS.md) · [solution](v3.8/SOLUTION.md) · pages `v3/report/v36_report.html`, `v36_findings.html` |
