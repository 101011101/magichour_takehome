...
| **v3.3 — LOCKED 2026-08-30 (unscored)** | can klein take the extraction call, so the deploy path is one model? **Yes, by eye:** A4 crop → klein head swap + dynamic pose+framing + garment hold → ankle cut → klein edit (+ one grounding sentence) is at parity with v3.1's locked arm on 24/28. Next: the scored iron-man test against every arm on disk ([solution §7](v3.3/SOLUTION.md#7-next-the-iron-man-test)). [experiment](v3.3/EXPERIMENT.md) · [matrix](v3.3/TEST.md) · [results](v3.3/RESULTS.md) · [solution](v3.3/SOLUTION.md) |
