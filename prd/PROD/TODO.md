...
| 10 | Is BiRefNet small, and will it be on the GPU? | **done — measured 2026-09-12; only crop parity open** | 44.6M params, 224 MB. It **does** run on the GPU with `onnxruntime-gpu==1.22.0` (the newest wheel fails at `dlopen`): BiRefNet alone **0.139 s vs 7.798 s — 56×**, and the **whole crop stage 0.58 s vs 8.14 s — 14×**, both ONNX models confirmed on CUDA. A garment costs **≈2.1 s** to prepare on GPU. Both notebooks pin that wheel first and fail closed. This also confirms the v3.9 crops ran on CPU. Still open: GPU crops have never been compared with the CPU references of record (T1) | `prd/v3/v3.8/BUILD.md` §5, §6.5; `vp/gpu_crop_time.py`; [OPEN Q4](OPEN_QUESTIONS.md) |
...
| 21 | Should a garment photo with no person in it skip the person-side stages? (Ray, 2026-09-12) | **decided — the person gate ships** | Yes. A nose landmark **or** a ≥500 px FACE region on the **upload** decides it; no person → no bald pass, no head crop, region forced to `full`, route recorded and in the cache key. Measured **69/69** worn photographs pass (56 fold + 13 on-model) and **1 of 17** distinct product shots misread as worn — zero false negatives, which is the direction that matters. It also removes the phantom behind the region caveat: the hip read fires on **1 of 10 uploads** but **6 of 10 bald frames**, because call 1 invents a wearer on a flat-lay | `v3/build/person_gate_check.py`; `v3/report/flatlay_hips.html`; [BUILD §6.1d](../v3/v3.8/BUILD.md); [OPEN Q7](OPEN_QUESTIONS.md) |
...
- [x] **Time the crop on the GPU.** Done 2026-09-12 with `vp/gpu_crop_time.py`: the whole
      stage **0.58 s on GPU against 8.14 s on CPU — 14×**, both ONNX models confirmed on
      CUDA, so a garment costs ≈2.1 s to prepare. Fills the ticket's crop-time blank.
      (Q4, ledger 10)
- [ ] **Prove crop parity (T1), writing its verdict to a file.** Still open, and now the
      one thing gating the GPU crop path: GPU crops have never been compared with the CPU
      references of record (MAD ≤ 4.0, shapes within 8 px). The 2026-09-12 inquiry attempt
      printed to the notebook and the session was released, so nothing was recorded. A few
      crops, no generation. (Q4, ledger 10)
